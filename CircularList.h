...
DoubleNode<T>* CircularList<T>::find(int index) 
{
   ...
   int min_dist;
   int dist_prev;  //prev links
   int dist_next;  //next links
   ...
   else
   {
      ...
      dist_next = (loc_pos - index - sze) * - 1;                             //distance using the bridge (next refs, positive)
   }
...
template < class T >
void CircularList<T>::draw(wxDC& dc, int width, int height)
{
   ...
   while(iter->hasNext())
   {
       ...
       if (before != NULL)
       {
          line->draw(dc, xbefore, ybefore, xd, yd);
       }
   ...
   if (before != NULL)
   {
      line->draw(dc, xd, yd, (int) (r + offset), (int) offset);
   }
   delete line;
